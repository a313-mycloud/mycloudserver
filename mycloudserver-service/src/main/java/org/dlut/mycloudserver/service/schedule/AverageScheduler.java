...
import org.dlut.mycloudserver.service.connpool.simpleconnpool.SimpleSingleHostConnPool;
...
public class AverageScheduler implements IScheduler {
    ...
    public Integer getBestHostId(VmDTO vmDTO) {
        Set<Integer> hostList = SimpleMutilHostConnPool.getRemoteMutilHostConnPoolMap().keySet();
        ...
        for (int hostId : hostList) {
            hostIdToVmCount.put(hostId, 0);
        }
        ...
        if (!result1.isSuccess()) {
            log.error("获取运行中的虚拟机列表失败，原因：" + result1.getMsgInfo());
            return null;
        }
